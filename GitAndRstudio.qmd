...
Adjacent to your console tab on the lower left, there is another tab called terminal. Go ahead and click it, and then using your mouse copy-paste the following lines of code individually, editing in your GitHub UserName, and email linked to your GitHub account:
...
While we are here, let's address the other segment where we will need the GitHub access token later. Go ahead and enter the following line of code:
...
This will open an .Renviron file that will be blank. Enter the following line of code, swapping in your specific token in its entirety between the quotation marks.
...
We want to make sure you can make changes to the file, and that these changes once saved can be committed and pushed to GitHub. Between the first line of text and the code block, add a couple spaces, and then save the file. 

Navigate to the upper right-side of your Rstudio window and click on Git. Under Staged tab, click the white box next to Examples_staff.qmd. Then hit the Commit button. A new window will appear. Write a brief commit message ("Testing that GitHub is setup correctly") and hit commit. Then click the green push up arrow. If you get a commit message, the system is set up correctly. Go back to GitHub in your browser, and check the commits, to see that the version control edits have been received and are available remotely. 
